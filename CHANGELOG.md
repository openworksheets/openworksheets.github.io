...
- **Puesta en marcha del servidor MCP desde la propia aplicación**. El cuadro de opciones de creación del editor (el que aparece al abrirlo sin ninguna ficha) incluye una opción nueva, **«Colocar campos con IA sobre un PDF…»**, con dos caminos según lo que pueda hacer cada programa. Si la IA ejecuta comandos (**Claude Code, Codex CLI, Antigravity CLI, Cursor, VS Code**), basta pegarle un encargo de tres líneas que apunta a la carpeta del servidor: ella la descarga, sigue su README, se registra y comprueba que responde. Si se usa **Claude Desktop, la aplicación de escritorio de ChatGPT o LM Studio**, que no pueden hacerlo, la aplicación **empaqueta el servidor en el navegador y lo ofrece como descarga en un clic** (1,5 MB) y da la configuración lista para pegar en su archivo, con la ruta ya puesta y las barras dobladas solas en Windows. El paso de descomprimir avisa de que la carpeta tiene que quedarse donde esté —el cliente guarda la ruta de `server.js` y lo ejecuta desde ahí cada vez—, y por eso propone la carpeta personal o Documentos antes que Descargas; el `.zip`, en cambio, se puede borrar. Cada programa va en un desplegable propio, cerrado, con un título que dice de cuál se trata («Claude Desktop — la aplicación de Claude para ordenador», «ChatGPT — la aplicación de escritorio (también vale para Codex CLI)»…): se abre solo el que interesa, en lugar de leer siete configuraciones seguidas. Dentro, cada uno lleva el menú exacto por el que se llega a su archivo —Claude Desktop (`claude_desktop_config.json`), ChatGPT de escritorio y Codex CLI (que comparten `~/.codex/config.toml`), LM Studio (`~/.lmstudio/mcp.json`), Antigravity —IDE y CLI— (`~/.gemini/config/mcp_config.json`), Cursor y VS Code—, su ruta y el bloque a copiar. Avisa además de que ChatGPT y Gemini **en su versión web** no sirven, porque solo admiten conectores remotos. El diálogo distingue la primera vez del resto: una vez se han copiado las instrucciones (o si se indica que ya está instalado), las siguientes veces se abre directamente en **«Pedirle una ficha a tu IA»**, con la petición lista para copiar y un par de ejemplos de ajustes («el campo 5 se ha quedado corto: ensánchalo…»), y la instalación queda a un clic por si hay que repetirla en otro ordenador. Como el servidor corre en local y la página no tiene forma de detectarlo, esa distinción se recuerda en el navegador. Disponible en los 5 idiomas.
