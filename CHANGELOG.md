...
### Cambiado
- **La vista previa del MCP es ahora la ficha de verdad, no un dibujo de los campos**. `preview_page` componía la imagen pintando rectángulos de colores sobre la página del PDF, y eso hacía que la IA diera por tapado lo que no lo estaba: un campo de opciones o una tabla ocupan solo lo que ocupa su contenido, así que si el rectángulo era más alto, la solución impresa debajo seguía viéndose en la ficha aunque en la vista previa pareciera cubierta. Ahora el servidor monta la página con el visor real del alumnado (`js/player.js`) en su Chromium sin interfaz y captura el resultado: lo que la IA mira es exactamente lo que verá quien haga la ficha, con sus campos, sus tipografías y sus tamaños reales. Encima solo se añaden el contorno y el número de cada campo para poder nombrarlos, y con `marks: false` se ve la página sin nada superpuesto. Si faltara la copia de la aplicación, la vista previa vuelve al dibujo de antes y lo dice en el aviso. El paquete lleva dentro `app/`, con los módulos del visor y sus estilos.
- **El diálogo del MCP se queda en una pantalla**. Instalar el servidor ya no es cosa del profesorado: se copian las instrucciones, se le pegan a la IA y ella descarga la carpeta, se registra y comprueba que responde. Lo hacen enteras Claude Code, Codex CLI, la aplicación de escritorio de ChatGPT, Antigravity, Cursor y VS Code; Claude Desktop y LM Studio, que no pueden tocar su propia configuración, dan ellos mismos el texto que hay que pegar y dicen dónde va. Así que sobraba todo lo que había debajo: la lista de ocho programas, cada uno con su ruta de configuración y con los tres estados en que podía estar su archivo (vacío, sin la clave, con la clave), y también la descarga de la carpeta empaquetada en el navegador. Eran veinticinco bloques de JSON que había que mantener al día y que envejecían mal —de hecho decían que ChatGPT de escritorio y Antigravity no podían instalarlo solos, y sí pueden—. Queda el encargo para la IA, los requisitos (Node 18 o superior y Chrome, Chromium o Edge) y los ejemplos de lo que pedirle después. El encargo, además, le dice ahora que si no puede registrarse sola avise de qué hay que pegar exactamente y en qué archivo. Los ejemplos de después también cambian: los dos primeros son las dos formas de empezar —convertir un PDF y crear una ficha desde cero sobre un tema— y ya no citan un archivo inventado; el de pedir cambios enseña ahora algo que solo se puede hacer así, cambiar el tipo de un ejercicio, en vez de recolocar un campo.
...
### Cambiado
- **Las dos opciones de IA dicen de qué parten y qué cuestan**. Sus nombres y descripciones contaban el procedimiento («Generar un prompt, pegar la respuesta…») en lugar de la diferencia entre ellas, que es lo único que necesita saber quien elige. Ahora son **«Crear una ficha con IA (prompt)»** —*Partiendo de un tema, copiando y pegando. Sin instalar nada*— y **«Crear o convertir fichas con IA (MCP)»** —*Desde un tema, un PDF o una imagen, y con vista previa. Hay que instalar un programa*—. Al poder el servidor MCP crear también desde cero, lo que separa las dos vías ya no es el punto de partida sino lo que alcanzan y lo que cuestan: el prompt no exige instalar nada y sirve en cualquier IA; el MCP añade partir de un documento, ver el resultado antes de guardar y pedir correcciones, a cambio de instalarse una vez.
...
### Añadido
- **Cuatro tipos de campo más para el servidor MCP**: **huecos en documento** (`textboxes`), **casillas** (`checkbox`), **tabla editable** (`table`) y **grabación de voz** (`record`). Los dos primeros colocan sus cajas directamente sobre la página —el rectángulo del campo se deduce solo de ellas, y la vista previa las dibuja numeradas—, lo que empareja «huecos en documento» con los huecos que ya detecta `read_layout`. En la tabla, cada celda admite varias respuestas válidas separadas por `|`. Con esto la IA puede colocar 18 de los tipos de campo.
- **`redact_areas`: borrar zonas de la página de verdad**. El campo decorativo «tapar zona» dibuja una capa en el visor, pero el contenido sigue dentro del paquete y se ve abriendo la imagen: no sirve para ocultar las soluciones impresas. Esta herramienta pinta sobre la propia imagen de fondo, de modo que lo tapado desaparece del archivo, y descarta además ese texto del análisis de la página para que no se proponga como enunciado.
- **«Archivo → Nuevo…»**: abre el cuadro *Empieza tu ficha* —el mismo que aparece con el editor vacío— para poder elegir cómo empezar sin tener que cerrar antes lo que se esté editando. Cada opción avisa si hay cambios sin guardar, igual que las del menú.
- **«Colocar campos con IA sobre un PDF (MCP)» en el menú Archivo**, junto a *Crear con IA…*: hasta ahora solo estaba en el cuadro de inicio, así que dejaba de verse en cuanto había una ficha abierta. Las cinco formas de empezar una ficha se definen ahora en un único sitio, de modo que el cuadro, el diálogo y el menú ofrecen siempre lo mismo.
- **Servidor MCP: crear fichas con ayuda de una IA a partir de un PDF** (`mcp/`). Una IA compatible con MCP (Claude y otras) puede ahora abrir un PDF del ordenador del profesor, leer su texto **con las coordenadas de cada línea**, colocar los campos de respuesta encima del documento, comprobar el resultado en una **vista previa con los campos dibujados y numerados**, corregir lo que esté desplazado y guardar el `.owpkg` listo para abrir en el editor. Las coordenadas van en fracciones de la página (0–1), el mismo sistema del manifiesto, y el PDF se rasteriza con el pdf.js del propio proyecto, de modo que la ficha es idéntica a la que saldría subiendo el PDF a mano. Detecta como candidatos a hueco las rachas de guiones bajos y las líneas horizontales impresas. Admite los campos `text`, `number`, `formula`, `essay`, `single`, `multi`, `select`, `truefalse`, `gaps`, `match`, `order` y `dragdrop` (**arrastrar a zonas**, en su modo de etiquetas: la IA coloca la bandeja y cada zona de destino con su respuesta, y la vista previa dibuja las zonas a trazos para poder comprobarlas), más los decorativos `label` y `cover`. Documentación en `mcp/README.md` y prueba de extremo a extremo en `tests/run_mcp_ficha.js`. Funciona con cualquier IA compatible con MCP que pueda ejecutar programas locales. No añade dependencias nuevas: solo Node, Chromium y el `puppeteer-core` que ya usaban los tests.
- **Puesta en marcha del servidor MCP desde la propia aplicación**. El cuadro de opciones de creación del editor (el que aparece al abrirlo sin ninguna ficha) incluye una opción nueva, **«Colocar campos con IA sobre un PDF…»**, con dos caminos según lo que pueda hacer cada programa. Si la IA ejecuta comandos (**Claude Code, Codex CLI, Antigravity CLI, Cursor, VS Code**), basta pegarle un encargo de tres líneas que apunta a la carpeta del servidor: ella la descarga, sigue su README, se registra y comprueba que responde. Si no puede —**Claude Desktop, la aplicación de escritorio de ChatGPT o LM Studio**—, o si simplemente se prefiere hacerlo a mano, la aplicación **empaqueta el servidor en el navegador y lo ofrece como descarga en un clic** (1,5 MB) y da la configuración lista para pegar en su archivo, con la ruta ya puesta y las barras dobladas solas en Windows. El paso de descomprimir avisa de que la carpeta tiene que quedarse donde esté —el cliente guarda la ruta de `server.js` y lo ejecuta desde ahí cada vez—, y por eso propone la carpeta personal o Documentos antes que Descargas; el `.zip`, en cambio, se puede borrar. El diálogo se organiza en dos apartados plegados, **«Si no has instalado el MCP»** y **«Si ya lo has instalado»**, para no leer lo que no toca: dentro del primero, los dos caminos —según la IA pueda ejecutar comandos o no— van a su vez plegados; el segundo recuerda que el servidor no se maneja desde OpenWorksheets, sino desde el propio programa de IA, y da la petición para convertir un PDF y un par de ejemplos de ajustes. En el camino manual, cada programa va en un desplegable propio, cerrado, con un título que dice de cuál se trata («Claude Desktop — la aplicación de Claude para ordenador», «ChatGPT — la aplicación de escritorio (también vale para Codex CLI)»…): se abre solo el que interesa, en lugar de leer ocho configuraciones seguidas. La lista termina con una entrada genérica, «Otro programa», para cualquier cliente MCP que no esté por su nombre, de modo que ningún texto necesita enumerar programas —esas listas siempre se quedaban cortas—. Dentro, cada uno lleva el menú exacto por el que se llega a su archivo —Claude Desktop (`claude_desktop_config.json`), ChatGPT de escritorio y Codex CLI (que comparten `~/.codex/config.toml`), LM Studio (`~/.lmstudio/mcp.json`), Antigravity —IDE y CLI— (`~/.gemini/config/mcp_config.json`), Cursor y VS Code—, su ruta y el fragmento a copiar para **cada uno de los tres estados en que puede estar el archivo**: vacío (se pega entero), con contenido pero sin la clave `mcpServers` (se pega en la segunda línea, justo bajo la primera llave) o con la clave ya puesta (se pega solo el servidor, debajo de esa línea). Saber dónde va exactamente es lo que no se puede adivinar, y pegar el bloque al final rompe el archivo. En el de TOML, en cambio, las líneas sí van al final, y también se dice. Avisa además de que ChatGPT y Gemini **en su versión web** no sirven, porque solo admiten conectores remotos. El diálogo abre siempre igual, por la configuración, y termina con lo que hay que pedirle a la IA una vez conectada: la petición para convertir un PDF y un par de ejemplos de ajustes sobre la marcha («el campo 5 se ha quedado corto: ensánchalo…»). Disponible en los 5 idiomas.
- **La carpeta `mcp/` es autónoma y no tiene dependencias**: lleva su propia copia de pdf.js (la misma que la aplicación) y habla con Chrome mediante un cliente de WebSocket y del protocolo CDP escrito para el caso (`mcp/cdp.js`, unas 300 líneas), en lugar de `puppeteer-core`. Así se descomprime y funciona: sin `npm install` y sin terminal, que es lo que hacía falta para que Claude Desktop o LM Studio —que no pueden ejecutar comandos ni configurarse solos— fueran una opción real. La prueba `tests/run_mcp_ficha.js` comprueba que ambas copias de pdf.js siguen siendo idénticas y admite `OWS_MCP_SERVER` para ejecutarse contra una copia instalada fuera del repositorio.
...
### Corregido
- **El servidor MCP no encontraba el navegador en Windows**: la lista de rutas solo contenía las de Linux y macOS, de modo que en Windows fallaba siempre salvo que se definiera a mano la variable `OWS_CHROME`. Ahora busca **Google Chrome, Chromium o Microsoft Edge** —vale cualquiera— por este orden: `OWS_CHROME`, `CHROME_PATH`, los ejecutables alcanzables desde el `PATH` (respetando `PATHEXT` en Windows) y las rutas habituales de instalación de cada sistema: en Windows, Chrome (también Beta, Dev y Canary), Chromium y Edge bajo `%PROGRAMFILES%`, `%PROGRAMFILES(X86)%` y `%LOCALAPPDATA%`, sin escribir ningún nombre de usuario; en macOS, `/Applications` y la carpeta del usuario; en Linux, las de siempre más `/snap/bin/chromium` y `microsoft-edge`. Si `OWS_CHROME` apunta a un archivo inexistente se avisa con esa ruta en lugar de seguir buscando en silencio, y cuando no hay ningún navegador el mensaje explica cuáles sirven y cómo indicar el suyo. La comprobación de permiso de ejecución solo se aplica donde significa algo, no en Windows. Prueba nueva en `tests/run_chrome_detect.js`, que simula las variables de entorno de cada sistema.
- **Los botones de copiar de los diálogos decían «Copiado» sin copiar nada**: cuando el navegador no concede permiso al portapapeles, la copia recurre a un cuadro de texto oculto, que colgaba de `<body>`; con un diálogo modal abierto todo lo que está fuera de él queda inerte, así que no se podía seleccionar y la copia fallaba en silencio —con el aviso de éxito igualmente—. Ahora ese cuadro se crea dentro del propio diálogo. Afectaba a «Copiar instrucciones para la IA» y a los bloques de configuración del servidor MCP, y también al «Copiar prompt» de *Crear con IA*.
- **Desplegables tapados al abrirlos**: en el visor del alumnado, la lista de opciones de un campo **Desplegable** quedaba oculta detrás de los campos situados más abajo en la página; solo se veía la del último. Cada campo es un elemento posicionado con `z-index` propio, de modo que crea su contexto de apilamiento y el `z-index` del menú solo competía dentro de su campo: entre campos ganaba el último del documento. Ahora el campo entero se eleva mientras su desplegable está abierto. Afecta también a las celdas desplegables de **Tabla editable**, que se comportaban igual.
