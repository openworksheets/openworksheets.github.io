...
- **Instalación del servidor MCP sin tocar nada: la hace la propia IA**. El cuadro de opciones de creación del editor (el que aparece al abrirlo sin ninguna ficha) incluye una opción nueva, **«Colocar campos con IA sobre un PDF…»**, que no da una configuración que copiar en un archivo, sino un **encargo listo para pegarle a la IA**: ella comprueba los requisitos (Node y Chrome), descarga la carpeta `mcp/`, instala su única dependencia, se registra como servidor MCP en la herramienta que se esté usando y comprueba que responde. El profesorado no edita ningún archivo de configuración ni se descarga la aplicación, que se sigue usando desde la web. Avisa además de que ChatGPT y Gemini en su versión web no pueden usar un servidor MCP local, porque solo admiten conectores remotos, y mantiene plegada la configuración manual (**Claude Desktop, LM Studio, Claude Code, Codex CLI, Antigravity CLI, Cursor, VS Code**) para quien la prefiera, con las rutas propias de cada sistema operativo y las barras dobladas solas en Windows. El diálogo distingue la primera vez del resto: una vez se han copiado las instrucciones (o si se indica que ya está instalado), las siguientes veces se abre directamente en **«Pedirle una ficha a tu IA»**, con la petición lista para copiar y un par de ejemplos de ajustes («el campo 5 se ha quedado corto: ensánchalo…»), y la instalación queda a un clic por si hay que repetirla en otro ordenador. Como el servidor corre en local y la página no tiene forma de detectarlo, esa distinción se recuerda en el navegador. Disponible en los 5 idiomas.
- **La carpeta `mcp/` es autónoma**: lleva su propia copia de pdf.js (la misma que la aplicación) y un `package.json` cuya única dependencia es `puppeteer-core`, de modo que puede descargarse sola —sin el resto del sitio— y funcionar. La prueba `tests/run_mcp_ficha.js` comprueba que ambas copias de pdf.js siguen siendo idénticas y admite `OWS_MCP_SERVER` para ejecutarse contra una copia instalada fuera del repositorio.
