...
> **Alternativa: exportar como página web autónoma.** Desde *Archivo → Exportar a web (ZIP)* la ficha se empaqueta como un ZIP con una copia del visor y un `index.html`. Basta con descomprimirlo y subir su contenido a cualquier alojamiento web propio para tenerla funcionando sin depender de OpenWorksheets ni de Google Drive. Conserva la contraseña de acceso y el cifrado de entrega de la ficha. El alumnado responde y, al terminar, descarga su archivo de entrega (`.owsub`) o copia el enlace de entrega: el propio `index.html` del paquete reconoce ese enlace y le muestra al docente la verificación de la entrega, de modo que la web es totalmente autónoma. Debe servirse por http(s): no funciona abriendo el `index.html` como archivo local.
