...
En el editor, «Colocar campos con IA sobre un PDF…» lleva de la mano: descarga el servidor en un clic (1,5 MB, empaquetados por la propia aplicación), se descomprime donde quieras y **no hay nada que instalar dentro** —no tiene dependencias—. Después, si tu IA puede ejecutar comandos (Claude Code, Codex CLI, Antigravity CLI, Cursor, VS Code) se le pega un texto y se configura sola; si usas **Claude Desktop o LM Studio**, que no pueden hacerlo, el mismo diálogo te da la configuración lista para pegar en su archivo, con la ruta ya puesta. Es cosa de una sola vez: a partir de ahí el diálogo se abre en «Pedirle una ficha a tu IA». ChatGPT y Gemini en su versión web no sirven, porque solo admiten conectores remotos. Detalles y límites en [`mcp/README.md`](mcp/README.md).
